...
trait httpRequest {
    ...
    public function closeRequest($param) {
        ...
        // ---- Early release logic ----
        // Send response headers only if not already sent
        if (!headers_sent()) {
        ...
        // Start output buffer and send content
        ignore_user_abort(true);  // Keep running if client disconnects
        set_time_limit(0);        // Avoid timeout on long tasks

        ob_start();
        echo $json;
        ob_end_flush();
        flush();  // Push response to client now
        // ---- At this point, the client connection is closed ----
        // You can continue with background processing below this line.
        // Example:
        // $this->doBackgroundWork();

        return '';  // Nothing else to output
    }
}
